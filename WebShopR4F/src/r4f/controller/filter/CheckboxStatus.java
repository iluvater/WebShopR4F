/**
 * 
 */
package r4f.controller.filter;

import java.util.List;

/**
 * @author Ture
 *
 */
public class CheckboxStatus {
	private String sport1;
	private String sport2;
	private String sport3;
	private String sport4;
	private String sport5;
	private String sport6;

	private String manufacturer1;
	private String manufacturer2;
	private String manufacturer3;
	private String manufacturer4;
	private String manufacturer5;
	private String manufacturer6;

	private String color1;
	private String color2;
	private String color3;
	private String color4;
	private String color5;
	private String color6;
	private String color7;
	private String color8;

	private String price1;
	private String price2;
	private String price3;
	private String price4;

	private String size1;
	private String size2;
	private String size3;
	private String size4;
	private String size5;
	private String size6;
	private String size7;
	private String size8;
	private String size9;
	private String size10;
	
	/**
	 * this constructor set all String to "unchecked"
	 */
	public CheckboxStatus() {
		sport1 = "unchecked";
		sport2 = "unchecked";
		sport3 = "unchecked";
		sport4 = "unchecked";
		sport5 = "unchecked";
		sport6 = "unchecked";
		manufacturer1 = "unchecked";
		manufacturer2 = "unchecked";
		manufacturer3 = "unchecked";
		manufacturer4 = "unchecked";
		manufacturer5 = "unchecked";
		manufacturer6 = "unchecked";
		color1 = "unchecked";
		color2 = "unchecked";
		color3 = "unchecked";
		color4 = "unchecked";
		color5 = "unchecked";
		color6 = "unchecked";
		color7 = "unchecked";
		color8 = "unchecked";
		price1 = "unchecked";
		price2 = "unchecked";
		price3 = "unchecked";
		price4 = "unchecked";
		size1 = "unchecked";
		size2 = "unchecked";
		size3 = "unchecked";
		size4 = "unchecked";
		size5 = "unchecked";
		size6 = "unchecked";
		size7 = "unchecked";
		size8 = "unchecked";
		size9 = "unchecked";
		size10 = "unchecked";
	}
	
	/**
	 * @return the sport1
	 */
	public String getSport1() {
		return sport1;
	}

	/**
	 * @param sport1
	 *            the sport1 to set
	 */
	public void setSport1(String sport1) {
		this.sport1 = sport1;
	}

	/**
	 * @return the sport2
	 */
	public String getSport2() {
		return sport2;
	}

	/**
	 * @param sport2
	 *            the sport2 to set
	 */
	public void setSport2(String sport2) {
		this.sport2 = sport2;
	}

	/**
	 * @return the sport3
	 */
	public String getSport3() {
		return sport3;
	}

	/**
	 * @param sport3
	 *            the sport3 to set
	 */
	public void setSport3(String sport3) {
		this.sport3 = sport3;
	}

	/**
	 * @return the sport4
	 */
	public String getSport4() {
		return sport4;
	}

	/**
	 * @param sport4
	 *            the sport4 to set
	 */
	public void setSport4(String sport4) {
		this.sport4 = sport4;
	}

	/**
	 * @return the sport5
	 */
	public String getSport5() {
		return sport5;
	}

	/**
	 * @param sport5
	 *            the sport5 to set
	 */
	public void setSport5(String sport5) {
		this.sport5 = sport5;
	}

	/**
	 * @return the sport6
	 */
	public String getSport6() {
		return sport6;
	}

	/**
	 * @param sport6
	 *            the sport6 to set
	 */
	public void setSport6(String sport6) {
		this.sport6 = sport6;
	}

	/**
	 * @return the manufacturer1
	 */
	public String getManufacturer1() {
		return manufacturer1;
	}

	/**
	 * @param manufacturer1
	 *            the manufacturer1 to set
	 */
	public void setManufacturer1(String manufacturer1) {
		this.manufacturer1 = manufacturer1;
	}

	/**
	 * @return the manufacturer2
	 */
	public String getManufacturer2() {
		return manufacturer2;
	}

	/**
	 * @param manufacturer2
	 *            the manufacturer2 to set
	 */
	public void setManufacturer2(String manufacturer2) {
		this.manufacturer2 = manufacturer2;
	}

	/**
	 * @return the manufacturer3
	 */
	public String getManufacturer3() {
		return manufacturer3;
	}

	/**
	 * @param manufacturer3
	 *            the manufacturer3 to set
	 */
	public void setManufacturer3(String manufacturer3) {
		this.manufacturer3 = manufacturer3;
	}

	/**
	 * @return the manufacturer4
	 */
	public String getManufacturer4() {
		return manufacturer4;
	}

	/**
	 * @param manufacturer4
	 *            the manufacturer4 to set
	 */
	public void setManufacturer4(String manufacturer4) {
		this.manufacturer4 = manufacturer4;
	}

	/**
	 * @return the manufacturer5
	 */
	public String getManufacturer5() {
		return manufacturer5;
	}

	/**
	 * @param manufacturer5
	 *            the manufacturer5 to set
	 */
	public void setManufacturer5(String manufacturer5) {
		this.manufacturer5 = manufacturer5;
	}

	/**
	 * @return the manufacturer6
	 */
	public String getManufacturer6() {
		return manufacturer6;
	}

	/**
	 * @param manufacturer6
	 *            the manufacturer6 to set
	 */
	public void setManufacturer6(String manufacturer6) {
		this.manufacturer6 = manufacturer6;
	}

	/**
	 * @return the color1
	 */
	public String getColor1() {
		return color1;
	}

	/**
	 * @param color1
	 *            the color1 to set
	 */
	public void setColor1(String color1) {
		this.color1 = color1;
	}

	/**
	 * @return the color2
	 */
	public String getColor2() {
		return color2;
	}

	/**
	 * @param color2
	 *            the color2 to set
	 */
	public void setColor2(String color2) {
		this.color2 = color2;
	}

	/**
	 * @return the color3
	 */
	public String getColor3() {
		return color3;
	}

	/**
	 * @param color3
	 *            the color3 to set
	 */
	public void setColor3(String color3) {
		this.color3 = color3;
	}

	/**
	 * @return the color4
	 */
	public String getColor4() {
		return color4;
	}

	/**
	 * @param color4
	 *            the color4 to set
	 */
	public void setColor4(String color4) {
		this.color4 = color4;
	}

	/**
	 * @return the color5
	 */
	public String getColor5() {
		return color5;
	}

	/**
	 * @param color5
	 *            the color5 to set
	 */
	public void setColor5(String color5) {
		this.color5 = color5;
	}

	/**
	 * @return the color6
	 */
	public String getColor6() {
		return color6;
	}

	/**
	 * @param color6
	 *            the color6 to set
	 */
	public void setColor6(String color6) {
		this.color6 = color6;
	}

	/**
	 * @return the color7
	 */
	public String getColor7() {
		return color7;
	}

	/**
	 * @param color7
	 *            the color7 to set
	 */
	public void setColor7(String color7) {
		this.color7 = color7;
	}

	/**
	 * @return the color8
	 */
	public String getColor8() {
		return color8;
	}

	/**
	 * @param color8
	 *            the color8 to set
	 */
	public void setColor8(String color8) {
		this.color8 = color8;
	}

	/**
	 * @return the price1
	 */
	public String getPrice1() {
		return price1;
	}

	/**
	 * @param price1
	 *            the price1 to set
	 */
	public void setPrice1(String price1) {
		this.price1 = price1;
	}

	/**
	 * @return the price2
	 */
	public String getPrice2() {
		return price2;
	}

	/**
	 * @param price2
	 *            the price2 to set
	 */
	public void setPrice2(String price2) {
		this.price2 = price2;
	}

	/**
	 * @return the price3
	 */
	public String getPrice3() {
		return price3;
	}

	/**
	 * @param price3
	 *            the price3 to set
	 */
	public void setPrice3(String price3) {
		this.price3 = price3;
	}

	/**
	 * @return the price4
	 */
	public String getPrice4() {
		return price4;
	}

	/**
	 * @param price4
	 *            the price4 to set
	 */
	public void setPrice4(String price4) {
		this.price4 = price4;
	}

	/**
	 * @return the size1
	 */
	public String getSize1() {
		return size1;
	}

	/**
	 * @param size1
	 *            the size1 to set
	 */
	public void setSize1(String size1) {
		this.size1 = size1;
	}

	/**
	 * @return the size2
	 */
	public String getSize2() {
		return size2;
	}

	/**
	 * @param size2
	 *            the size2 to set
	 */
	public void setSize2(String size2) {
		this.size2 = size2;
	}

	/**
	 * @return the size3
	 */
	public String getSize3() {
		return size3;
	}

	/**
	 * @param size3
	 *            the size3 to set
	 */
	public void setSize3(String size3) {
		this.size3 = size3;
	}

	/**
	 * @return the size4
	 */
	public String getSize4() {
		return size4;
	}

	/**
	 * @param size4
	 *            the size4 to set
	 */
	public void setSize4(String size4) {
		this.size4 = size4;
	}

	/**
	 * @return the size5
	 */
	public String getSize5() {
		return size5;
	}

	/**
	 * @param size5
	 *            the size5 to set
	 */
	public void setSize5(String size5) {
		this.size5 = size5;
	}

	/**
	 * @return the size6
	 */
	public String getSize6() {
		return size6;
	}

	/**
	 * @param size6
	 *            the size6 to set
	 */
	public void setSize6(String size6) {
		this.size6 = size6;
	}

	/**
	 * @return the size7
	 */
	public String getSize7() {
		return size7;
	}

	/**
	 * @param size7
	 *            the size7 to set
	 */
	public void setSize7(String size7) {
		this.size7 = size7;
	}

	/**
	 * @return the size8
	 */
	public String getSize8() {
		return size8;
	}

	/**
	 * @param size8
	 *            the size8 to set
	 */
	public void setSize8(String size8) {
		this.size8 = size8;
	}

	/**
	 * @return the size9
	 */
	public String getSize9() {
		return size9;
	}

	/**
	 * @param size9
	 *            the size9 to set
	 */
	public void setSize9(String size9) {
		this.size9 = size9;
	}

	/**
	 * @return the size10
	 */
	public String getSize10() {
		return size10;
	}

	/**
	 * @param size10
	 *            the size10 to set
	 */
	public void setSize10(String size10) {
		this.size10 = size10;
	}
}
